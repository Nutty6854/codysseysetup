...
- [x] VS Code GitHub 로그인 및 저장소 연동 증거 첨부
- [x] 실제 사용한 터미널 앱 이름 기록

> **포트 접속 증거 제출 결정:** 주소창의 `localhost:8080`이 포함된 브라우저 재캡처는 생략하고, 기존 웹페이지 화면과 `curl` 응답, `HTTP/1.1 200 OK`, NGINX 접근 로그를 제출합니다. 과제 원문의 주소창 포함 요구사항과는 차이가 있어 평가 시 감점 가능성이 있습니다.
...
- 과제 원문은 브라우저 주소창의 `localhost:8080`과 응답 화면이 함께 보이는 증거를 요구하지만, 이번 제출에서는 기존 화면과 `curl`·`200 OK`·NGINX 로그로 대체합니다.
